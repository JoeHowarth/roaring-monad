use futures::executor::block_on;
use monad_chain_data::{
    Address, B256, Bytes, FinalizedBlock, InMemoryBlobStore, InMemoryMetaStore, Log, LogData,
    LogFilter, MonadChainDataService, QueryLogsRequest, QueryOrder,
};

#[test]
fn query_logs_paginates_at_block_boundaries() {
    block_on(async {
        let service =
            MonadChainDataService::new(InMemoryMetaStore::default(), InMemoryBlobStore::default());

        service
            .ingest_block(FinalizedBlock {
                block_number: 1,
                block_hash: B256::repeat_byte(1),
                parent_hash: B256::ZERO,
                logs_by_tx: vec![vec![
                    log(Address::repeat_byte(7), B256::repeat_byte(9)),
                    log(Address::repeat_byte(7), B256::repeat_byte(9)),
                ]],
            })
            .await
            .expect("ingest block 1");

        service
            .ingest_block(FinalizedBlock {
                block_number: 2,
                block_hash: B256::repeat_byte(2),
                parent_hash: B256::repeat_byte(1),
                logs_by_tx: vec![vec![log(Address::repeat_byte(7), B256::repeat_byte(9))]],
            })
            .await
            .expect("ingest block 2");

        let first_page = service
            .query_logs(QueryLogsRequest {
                from_block: Some(1),
                to_block: Some(2),
                order: QueryOrder::Ascending,
                limit: 1,
                filter: LogFilter {
                    address: Some(Address::repeat_byte(7)),
                    topic0: Some(B256::repeat_byte(9)),
                },
            })
            .await
            .expect("first page");

        assert_eq!(first_page.logs.len(), 2);
        assert_eq!(first_page.cursor_block.number, 1);
        assert_eq!(first_page.logs[0].block_number, 1);
        assert_eq!(first_page.logs[1].block_number, 1);

        let second_page = service
            .query_logs(QueryLogsRequest {
                from_block: Some(first_page.cursor_block.number + 1),
                to_block: Some(2),
                order: QueryOrder::Ascending,
                limit: 1,
                filter: LogFilter {
                    address: Some(Address::repeat_byte(7)),
                    topic0: Some(B256::repeat_byte(9)),
                },
            })
            .await
            .expect("second page");

        assert_eq!(second_page.logs.len(), 1);
        assert_eq!(second_page.cursor_block.number, 2);
        assert_eq!(second_page.logs[0].block_number, 2);
    });
}

#[test]
fn query_logs_descending_returns_newest_first() {
    block_on(async {
        let service =
            MonadChainDataService::new(InMemoryMetaStore::default(), InMemoryBlobStore::default());

        service
            .ingest_block(FinalizedBlock {
                block_number: 1,
                block_hash: B256::repeat_byte(1),
                parent_hash: B256::ZERO,
                logs_by_tx: vec![vec![
                    log(Address::repeat_byte(5), B256::repeat_byte(8)),
                    log(Address::repeat_byte(5), B256::repeat_byte(8)),
                ]],
            })
            .await
            .expect("ingest");

        let page = service
            .query_logs(QueryLogsRequest {
                from_block: Some(1),
                to_block: Some(1),
                order: QueryOrder::Descending,
                limit: 1,
                filter: LogFilter {
                    address: Some(Address::repeat_byte(5)),
                    topic0: Some(B256::repeat_byte(8)),
                },
            })
            .await
            .expect("query");

        assert_eq!(page.logs.len(), 2);
        assert_eq!(page.logs[0].log_index, 1);
        assert_eq!(page.logs[1].log_index, 0);
    });
}

fn log(address: Address, topic0: B256) -> Log {
    Log {
        address,
        data: LogData::new_unchecked(vec![topic0], Bytes::from(vec![1, 2, 3])),
    }
}
